    ...
    socket.on("register_user", ({ userId, deviceId }) => {
      if (userId) {
        ...
        if (existingData && existingData.deviceId !== deviceId) {
          ...
          if (existingSocket) {
            console.log(
              `Force logout user ${userId} from device ${existingData.deviceId} (socket ${existingData.socketId})`,
            );
          ...
          console.log(
            `User ${userId} reconnected from same device ${deviceId}, updating socket`,
          );
        }
        ...
        console.log(
          `User ${userId} registered with socket ${socket.id}, device ${deviceId}, joined room ${userId}`,
        );
      }
    });
